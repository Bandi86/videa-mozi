...
if (process.env.STORAGE_PROVIDER === 'LOCAL' && process.env.LOCAL_STORAGE_PATH) {
  app.use('/uploads', express.static(process.env.LOCAL_STORAGE_PATH))
}
...
async function startServer(): Promise<void> {
  try {
    ...
    server.listen(PORT, () => {
      ...
      logger.info(`🚀 Media Service running on ${protocol}://localhost:${PORT}`)
